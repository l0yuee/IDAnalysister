...
- **调用点参数提取**，覆盖需求文档中列出的每一种 x86/x86-64 参数传递形式：寄存器直接传递、立即数直接传递、内存间接传递（永远解引用到真实值，而不是仅返回地址）、`push` 序列传参（正确处理从右到左的顺序）、通过 `mov` 写栈传参、寄存器间接寻址/结构体字段访问、全局变量与 TLS 访问，以及"前一次调用的返回值被用作参数"这种跨调用的数据流；`xchg`、`cmov`、`movsx`、`movzx`、`lea`、`add`、`sub` 等形式开箱即支持，其他任何形式都可以在不修改核心代码的前提下自行添加。
