...
**The adapter must undo IDA's lossy operand encoding before `core/` ever sees it.** `adapters/ida_operand_decode.py` is pure (no `ida_*` import, unit-tested) and holds two reconstructions that are silently wrong if skipped: (1) for x86 `o_phrase`/`o_displ` operands with a SIB byte, `op_t.reg`/`phrase` holds the ModRM "SIB follows" marker (`R_sp`, +8 with `REX.B`), *not* the base — the real base/index/scale come from `op_t.specflag2` plus the REX bits in `insn_t.insnpref`, so `[ebx+ecx*4]` decodes as `[esp+0]` if taken at face value; (2) IDA widens `op_t.addr` (displacement) and `op_t.value` (immediate) to sign-extended 64-bit values regardless of database bitness, so `[ebp-8]` arrives as `0xFFFFFFFFFFFFFFF8` and `push -4` as `0xFFFFFFFFFFFFFFFC` in a 32-bit IDB. Displacements are normalized to signed ints; immediates to the unsigned bit pattern of `dtype_size`; arithmetic folding in `locators/arithmetic_locators.py` wraps results with `locators._common.wrap_to_width`.
